...
const getDocAppointmentList=async()=>{
  let result = await docAppointmentListService(searchForm.value)
...
const addDocAppointment=async()=>{
  ...
  const processedForm = {
    date: form.date instanceof Date ? form.date.toISOString().substring(0, 10) : '', // 如果是日期对象，转换为字符串
...
<template>
  <div>
    ...
    <el-button type="primary" @click="addDocAppointmentForm = true">add</el-button>
      ...
      <el-dialog v-model="addDocAppointmentForm" title="添加排班信息" width="500">
